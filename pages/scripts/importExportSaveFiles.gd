...
func legacyLoad(path): #when you pick a legacy save file. I'm gonna be 100% honest, this feature is completely NOT nessicary, but you know... It's the little things that count.
	...
	if FileAccess.file_exists(path) == true: #if the file actually exists (don't know how you would have picked it if it didn't but, y'know. Crash prevention. You know how it is. Yes you do.
		...
		if file: #if the file is valid
			...
			print("it failed dawg. what did you just try to load")
...
func _on_import_legacy_save_pressed() -> void: #opens and closes the file picker
	$fileDialog.visible = true

func _on_file_dialog_file_selected(path: String) -> void: #when you pick a legacy save file
	legacyLoad(path)
