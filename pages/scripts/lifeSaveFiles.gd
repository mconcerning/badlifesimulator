...
func _ready() -> void:
	...
	for i in dir.size(): #runs through every file in the directory
		...
		button.position = Vector2(1080.0 / 2 - float(button.size.x) / 2, positionDown) #sets its position to be centred horizontally and positionDown pixels lower on the y axis than the top of the scrollContainer
